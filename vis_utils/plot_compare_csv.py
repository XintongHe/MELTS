...
def load_progress(progress_csv_path):
    ...
    with open(progress_csv_path, 'r') as csvfile:
        reader = csv.DictReader((line.replace('\0','') for line in csvfile), delimiter=delimiter)
        for row in reader:
...
def main(paths, names=None, title_='Cheetah 8-Task', top_limit=None, bottom_limit=None, x_lefts=None, x_rights=None, plot_type=0, save_=True):
    ...
    bottom_limit = None if bottom_limit is None or bottom_limit == "" else [float(el) for el in bottom_limit.split("||")]
    x_rights = None if x_rights is None or x_rights == "" else [float(el) for el in x_rights.split("||")]
    x_lefts = None if x_lefts is None or x_lefts == "" else [float(el) for el in x_lefts.split("||")]
    ...
    for data_dict in dd_list:
        ...
        for name, (values, keys) in data_dict.items():
            ...
            for (s, t) in [('AverageReturn_all_test_tasks', 'Average Reward Test'),
                           ...
                           # ('train-AverageReturn', 'Average Reward Test'),
                           # ('classification_acc', 'Unsupervised Classification Accuracy')]:
                if s in keys:
                    for v in values:
    ...
    for title, values in pl_list[0].items():
        ...
        for i, plotting_data in enumerate(pl_list):
            ...
            min_steps_total = np.max([np.min([np.max(s[0]) for s in plotting_data[title][method_name]])
            ...
            if top_limit is not None and top_limit[i % len(top_limit)] != -1:
                axs[i].set_ylim(top=top_limit[i % len(top_limit)])
            if bottom_limit is not None and bottom_limit[i % len(bottom_limit)] != -1:
                axs[i].set_ylim(bottom=bottom_limit[i % len(bottom_limit)])
            ...
            if i == 0:
                axs[i].set_ylabel('Average Return $\it{R}$')
                # axs[i].set_ylabel('Classification Accuracy')
            axs[i].semilogx()

            if x_rights is not None and x_rights[i % len(x_rights)] > 0:
                axs[i].set_xlim(right=x_rights[i % len(x_rights)])
            if x_lefts is not None and x_lefts[i % len(x_lefts)] > 0:
                axs[i].set_xlim(left=x_lefts[i % len(x_lefts)])

            # if axs[i].get_xlim()[0] > 10**4:
            #     axs[i].set_xlim(left=2*10**5)
        ...
        if plot_type == 0:
            ...
            leg = fig.legend(handles=axs[-1].get_legend_handles_labels()[0], labels=axs[-1].get_legend_handles_labels()[1],
                       bbox_to_anchor=(0.5, -0.08), loc='upper center',
                             ncol=len(pl_list[-1][title].keys()) + 1 if len(pl_list) > 1 else 3,
                             handlelength=1.5)
